...
local function UpdatePortraitColor(self, unit, min, max)
    ...
    local function TeccUnitColor(unit)

        if (config.show.reactionHealth) then
        ...
        if UnitIsPlayer(unit) then
            if not UnitIsConnected(unit) then
                r, g, b = 0.5, 0.5, 0.5;
            elseif UnitClass(unit) and (config.show.classHealth) then
    ...
    if not UnitIsConnected(unit) then
        ...
        if config.show.classHealth then
            if UnitIsPlayer(unit) then
                ...
                Health:SetStatusBarColor(TeccUnitColor("target"));
            end
        else
...
local function CreateUnitLayout(self, unit)
    ...
    if unit == "player" then
        ...
        self.Health:SetPoint("TOPLEFT", self.Texture, 106, -24)
    elseif unit == "pet" then
        self.Health:SetSize(70, 9)
        self.Health:SetPoint("TOPLEFT", self.Texture, 45, -20)
    elseif unit == "target" or unit == "focus" then
        self.Health:SetSize(119, 28)
        self.Health:SetPoint("TOPRIGHT", self.Texture, -105, -24) -- -105
    elseif self.IsTargetFrame then
    ...
    elseif unit == "target" or unit == "focus" then
        self.Name:SetWidth(110)
        self.Name:SetPoint("CENTER", self.Texture, -50, 36)
    elseif self.IsTargetFrame then
